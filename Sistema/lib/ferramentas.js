/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Este arquivo é IDÊNTICO nos dois sistemas (5K9 Forms e 5K9 Gestor). A
   única linha que muda entre eles é a constante ATUAL, no fim. Ao criar uma
   ferramenta nova, acrescente uma entrada aqui e copie o arquivo para os
   outros repositórios — é a lista que faz cada sistema saber que os outros
   existem.

   Poderia ser um endpoint compartilhado, mas não deveria: um menu de
   navegação que depende de rede fica vazio quando a rede falha, e o custo
   de manter três linhas em sincronia é menor que o de explicar por que o
   botão de trocar de ferramenta às vezes não lista nada.
   ═══════════════════════════════════════════════════════════════════════════ */
 ...
 *
 * Em desenvolvimento os dois sistemas rodam em portas diferentes do mesmo
...
export const urlDe = (ferramenta) => {
    ...
    /* Em produção, monta o host trocando o primeiro rótulo do domínio atual
       ...
       mudar: gestor.5k9.studio → painel.5k9.studio, sem este arquivo
